...
`,p=Object.assign({"../content/blog/2023-09-displaying-sharepoint-version-history-in-power-apps.md":h}),u=t=>{const e=t.trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.ceil(e/200))},d=t=>typeof t!="string"?new Date().toISOString().slice(0,10):t,l=t=>{const e=t.trim();return e==="true"?!0:e==="false"?!1:e.startsWith('"')&&e.endsWith('"')||e.startsWith("'")&&e.endsWith("'")?e.slice(1,-1):e},m=t=>{const e=t.trim().replace(/^\[/,"").replace(/\]$/,"");return e?e.split(",").map(o=>l(o)).filter(o=>typeof o=="string"&&o.length>0):[]},f=t=>{if(!t.startsWith("---"))return{data:{},content:t.trim()};const e=t.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);if(!e)return{data:{},content:t.trim()};const[,o,i]=e,r={};for(const n of o.split(/\r?\n/)){if(!n.trim())continue;const s=n.indexOf(":");if(s===-1)continue;const c=n.slice(0,s).trim(),a=n.slice(s+1).trim();r[c]=a.startsWith("[")?m(a):l(a)}return{data:r,content:i.trim()}},w=()=>Object.entries(p).map(([t,e])=>{const o=t.split("/").pop()?.replace(/\.md$/,"")??"post",r=f(typeof e=="string"?e:""),n=r.data;return{slug:o,title:n.title??o,date:d(n.date),tags:Array.isArray(n.tags)?n.tags:[],summary:n.summary??"",coverImage:n.coverImage,draft:n.draft??!1,content:r.content.trim(),readingTimeMinutes:u(r.content)}}).filter(t=>!t.draft).sort((t,e)=>e.date.localeCompare(t.date)),y=t=>w().find(e=>e.slug===t);export{y as a,w as g};
