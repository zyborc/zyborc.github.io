...
`,f=Object.assign({"../content/blog/2023-09-displaying-sharepoint-version-history-in-power-apps.md":l,"../content/blog/2026-03-power-apps-code-apps-getting-started.md":d,"../content/blog/2026-03-power-apps-code-apps-lessons-learned.md":u,"../content/blog/2026-03-power-apps-copilot-studio-pitfalls.md":h}),m=n=>{const e=n.trim().split(/\s+/).filter(Boolean).length;return Math.max(1,Math.ceil(e/200))},g=n=>typeof n!="string"?new Date().toISOString().slice(0,10):n,c=n=>{const e=n.trim();return e==="true"?!0:e==="false"?!1:e.startsWith('"')&&e.endsWith('"')||e.startsWith("'")&&e.endsWith("'")?e.slice(1,-1):e},y=n=>{const e=n.trim().replace(/^\[/,"").replace(/\]$/,"");return e?e.split(",").map(o=>c(o)).filter(o=>typeof o=="string"&&o.length>0):[]},w=n=>{if(!n.startsWith("---"))return{data:{},content:n.trim()};const e=n.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);if(!e)return{data:{},content:n.trim()};const[,o,i]=e,s={};for(const t of o.split(/\r?\n/)){if(!t.trim())continue;const a=t.indexOf(":");if(a===-1)continue;const p=t.slice(0,a).trim(),r=t.slice(a+1).trim();s[p]=r.startsWith("[")?y(r):c(r)}return{data:s,content:i.trim()}},v=()=>Object.entries(f).map(([n,e])=>{const o=n.split("/").pop()?.replace(/\.md$/,"")??"post",s=w(typeof e=="string"?e:""),t=s.data;return{slug:o,title:t.title??o,date:g(t.date),tags:Array.isArray(t.tags)?t.tags:[],summary:t.summary??"",coverImage:t.coverImage,draft:t.draft??!1,content:s.content.trim(),readingTimeMinutes:m(s.content)}}).filter(n=>!n.draft).sort((n,e)=>e.date.localeCompare(n.date)),b=n=>v().find(e=>e.slug===n);export{b as a,v as g};
